...
export default function OngoingCard({ title, time, days }: OngoingCard) {
  ...
  return (
    <div
      className="flex flex-col gap-y-2 shadow-lg px-4 py-4 rounded-xl"
      onClick={() => router.push("/dashboard/task")}
    >
      <div className="flex justify-between">
        <h1 className="font-bold text-[20px]">{title}</h1>
        <button className="bg-[#D2691E] w-[50px] rounded-3xl text-[#FFFFFF]">
          {days}
        </button>
      </div>
      <div className="flex flex-col gap-y-2">
        <h1 className="text-[grey]">Team members</h1>
